...
138 tests across 15 files. Run with `pnpm run test` (vitest).

## Structure

### Unit tests (`tests/unit/`)
14 files testing individual modules in isolation:
...
1 file: `scan-examples.test.ts` — scans real example log files from `tests/example-logs/` and validates encounter counts, boss names, and durations against known-good values.
