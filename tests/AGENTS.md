...
168 tests across 17 files. Run with `pnpm run test` (vitest).
...
| `combat-tracker.test.ts` | Combat stats tracking (damage, healing, overkill, pet resolution, friendly-fire exclusion, encounter lifecycle) |
...
- `parse-combat-stats.test.ts` — validates per-player damage/healing on Patchwerk and Razuvious against uwu-logs reference numbers (within 2% tolerance).
